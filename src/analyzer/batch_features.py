...
def run_batch_feature_extraction():
    # 1. Швидке зчитування хакатонів (БД закривається одразу після цього)
    con = get_connection()
    try:
    ...
    logger.info("Завантаження Sentence-BERT (MiniLM)...")
    ...
    for _, h in hackathons_df.iterrows():
        ...
        # 2. Швидке зчитування проектів для конкретного хакатону (БД закривається одразу)
        con = get_connection()
        try:
        ...
        # Розрахунок ембеддингів та PCA виконується у процесорі (БД у цей час ПОВНІСТЮ вільна!)
        ...
        # 3. Швидкий запис пакету ознак в БД (відкриваємо строго на мікросекунди)
        con = get_connection()
        try:
            con.execute("BEGIN")
            ...
            for i, (_, p) in enumerate(projects_df.iterrows()):
                ...
                """, [
                    ...
                    f["github_stars"], f["repo_size"], f["repo_issues"], f["days_before_deadline"], 
                    f["prize_per_team"], f["organizer_reputation"]
                ])
            con.commit()
