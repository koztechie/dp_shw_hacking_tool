...
def _get_rag_context() -> str:
    ...
    АНТИКРИХКІСТЬ: Сортування за likes DESC замість RANDOM(), щоб RAG-текст
    був детермінованим і на 100% кешувався платформою MiMo.
    """
    try:
...
def generate_winning_ideas(hackathon_data: dict, hackathon_analysis: dict, hard_constraints: dict) -> list[dict]:
    """
    Мульти-агентна генерація ідей: RAG -> Brainstorming -> Critic.
    ОПТИМІЗАЦІЯ КЕШУ: Усі інструкції, схеми та тренди йдуть НА ПОЧАТКУ.
    """
    ...
    if trends_file.exists():
        try: global_trends = trends_file.read_text(encoding="utf-8")
        except Exception: pass

    # ==========================================
    # АГЕНТ 1: CREATIVE BRAINSTORMER (Кешований Промпт)
    # ==========================================
    logger.info("Агент 1: Аналіз RAG-контексту та генерація пулу ідей...")
    draft_prompt = f"""You are an avant-garde product architect. Brainstorm 5 highly innovative project ideas for a hackathon.
...
=========================================
NEW HACKATHON TARGET (Process this specific data):
{json.dumps(hackathon_data, ensure_ascii=False)[:1000]}
"""
    ...
    # ==========================================
    # АГЕНТ 2: ADVERSARIAL CRITIC (Кешований Промпт)
    # ==========================================
    logger.info("Агент 2: Робота Суворого Критика (фільтрація обмежень та фіналізація)...")
    critic_prompt = f"""You are an extremely strict Hackathon Judge and Technical Validator.
...
if __name__ == "__main__":
    print("=== ТЕСТУВАННЯ PREFIX CACHING ПАТЕРНІВ ===")
    mock_data = {"title": "Web3 Future Hack"}
    ideas = generate_winning_ideas(mock_data, {}, {})
