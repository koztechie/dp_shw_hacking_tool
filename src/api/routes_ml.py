...
from src.api.core import templates, AppState, limiter, verify_local_access, get_workflow_context, PROJECT_ROOT
...
async def ideas_page(request: Request, prediction_id: str):
    try:
        ...
        row = con.execute("SELECT hackathon_url, idea_1_description, idea_2_description, idea_3_description FROM predictions WHERE id = ?", [prediction_id]).fetchone()
    ...
    if not row: return HTMLResponse("Передбачення не знайдено (404)", status_code=404)
    ...
    return templates.TemplateResponse(request=request, name="ideas.html", context={"prediction_id": prediction_id, "ideas": valid_ideas, "hackathon_url": row[0], "workflow": workflow, "t": t})
...
async def get_techspec(request: Request, prediction_id: str, idea_index: int):
    ...
    techspec = generate_and_save_techspec(prediction_id, idea_index, row[0] if row else "")
    if "error" in techspec: return JSONResponse({"error": techspec["error"]}, status_code=500)
...
async def techspec_page(request: Request, prediction_id: str):
    ...
    return templates.TemplateResponse(request=request, name="techspec.html", context={"techspec": techspec, "selected_idea": row[1] if row else None, "workflow": workflow, "t": t})
...
async def history_page(request: Request, page: int = 1, limit: int = 50):
    ...
    try:
        ...
        import pandas as pd
        con = duckdb.connect(DB_PATH, read_only=True)
        df = con.execute("SELECT p.id, p.hackathon_url, strftime('%Y-%m-%d %H:%M', p.generated_at) as gen_date, p.idea_1_title, p.idea_1_score, p.selected_idea, f.won as feedback_won FROM predictions p LEFT JOIN feedback f ON p.id = f.prediction_id ORDER BY p.generated_at DESC LIMIT ? OFFSET ?", [limit, offset]).fetchdf()
        records = df.to_dict("records")
        for r in records:
            r["feedback_won"] = None if pd.isna(r.get("feedback_won")) else bool(r.get("feedback_won"))
        predictions = records
    except Exception as e:
        logger.error(f"Помилка завантаження історії: {e}")
    finally:
        if "con" in locals(): con.close()
    return templates.TemplateResponse(request=request, name="history.html", context={"predictions": predictions, "page": page, "limit": limit, "t": t})
...
async def recommend_hackathon(request: Request):
    try:
        ...
        if not result: return JSONResponse({"error": "Не знайдено жодного відповідного хакатону."}, status_code=404)
...
async def retrain_check():
    ...
    if count_file.exists():
        with contextlib.suppress(BaseException): last_count = int(count_file.read_text(encoding="utf-8").strip())
    ...
    if cooldown_file.exists():
        with contextlib.suppress(BaseException):
            if _time.time() - float(cooldown_file.read_text(encoding="utf-8").strip()) < 600:
                return JSONResponse({"hackathons": current_count, "last_train_count": last_count, "suggest_retrain": False})

    if not has_model: suggest_retrain = True
    elif current_count > last_count:
        delta = current_count - last_count
        if delta >= 20: suggest_retrain = True
        else:
            from src.ml.drift_detector import detect_drift
            if delta >= 5 and detect_drift(): suggest_retrain = True

    return JSONResponse({"hackathons": current_count, "last_train_count": last_count, "suggest_retrain": suggest_retrain})
...
def run_ml_pipeline():
    try:
        ...
        import time as _time
        (PROJECT_ROOT / "data" / "models" / "last_train_time.txt").write_text(str(_time.time()), encoding="utf-8")
    except Exception as e:
...
async def submit_feedback(prediction_id: str, background_tasks: BackgroundTasks, won: bool = Form(...), actual_place: int = Form(0)):
    try:
        import duckdb
        con = duckdb.connect(DB_PATH)
        if not con.execute("SELECT id FROM predictions WHERE id = ?", [prediction_id]).fetchone():
...
async def generate_assets(prediction_id: str):
    try:
        ...
        if "con" in locals(): con.close()
    if not row or not row[0]: return JSONResponse({"error": "TechSpec not found."}, status_code=404)
    try:
