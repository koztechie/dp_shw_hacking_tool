...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    logger.error(
        f"🚨 Unhandled Exception (ID: {error_id}) at {request.url.path}: {exc}"
    )
    try:
        import sentry_sdk
...
async def custom_rate_limit_handler(request: Request, exc: RateLimitExceeded):
    ...
    return JSONResponse(
        ...
        content={
            "status": "error",
            "error": "Занадто багато запитів. Будь ласка, зачекайте.",
        },
    )
...
async def dashboard(request: Request):
    ...
    if not onboarding_flag.exists():
        return templates.TemplateResponse(
            request=request, name="onboarding.html", context={"t": t}
        )
    ...
    try:
        ...
        stats["winners"] = con.execute(
            "SELECT COUNT(*) FROM projects WHERE is_winner=TRUE"
        ).fetchone()[0]
        if stats["projects"] > 0:
            stats["win_rate"] = int((stats["winners"] / stats["projects"]) * 100)
        AppState.set_count(stats["hackathons"])
        with contextlib.suppress(Exception):
            stats["predictions"] = con.execute(
        ...
        if "con" in locals():
            con.close()
    return templates.TemplateResponse(
        request=request, name="index.html", context={"stats": stats, "t": t}
    )
...
async def health(request: Request):
    health_status = {
        ...
        "components": {
            ...
            "ai_api": {"status": "unknown"},
            "disk_space": {"status": "unknown"},
            "memory": {"status": "unknown"},
            "cpu": {"status": "unknown"},
        },
        "metrics": {"hackathons": 0, "projects": 0, "predictions": 0},
    }
    critical_failures = []
    ...
    try:
        ...
        health_status["metrics"]["projects"] = con.execute(
            "SELECT COUNT(*) FROM projects"
        ).fetchone()[0]
        health_status["metrics"]["predictions"] = con.execute(
            "SELECT COUNT(*) FROM predictions"
        ).fetchone()[0]
    ...
    try:
        ...
        health_status["components"]["disk_space"] = {
            ...
            else ("warning" if disk_usage.percent < 90 else "critical"),
            "details": {
        ...
        health_status["components"]["memory"] = {
            ...
            else ("warning" if mem_usage.percent < 90 else "critical"),
            "details": {
                "used_percent": round(mem_usage.percent, 1),
        ...
        health_status["components"]["cpu"] = {
            "status": "ok"
            if cpu_percent < 85
            else ("warning" if cpu_percent <= 95 else "degraded"),
    ...
    if critical_failures:
        health_status["status"] = (
            "critical"
            if any(f in critical_failures for f in ["database", "ml_model", "memory"])
            else "degraded"
        )
        health_status["critical_failures"] = critical_failures

    return JSONResponse(
        health_status, status_code=503 if health_status["status"] == "critical" else 200
    )
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        ...
        reload=is_dev,
        workers=1 if not is_dev else None,
    )
